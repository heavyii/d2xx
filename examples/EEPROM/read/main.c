...
int main(int argc, char *argv[])
{
	...
	if (ftDevice == FT_DEVICE_232H)
	{
		...
		printf("232H:\n");
		printf("-----\n");
		printf("\tPullDownEnableH = 0x%X\n", Data.PullDownEnableH);				//UCHAR PullDownEnableH;		// non-zero if pull down enabled
		printf("\tSerNumEnableH = 0x%X\n", Data.SerNumEnableH);				// UCHAR SerNumEnableH;		// non-zero if serial number to be used
		printf("\tACSlowSlewH = 0x%X\n", Data.ACSlowSlewH);				//UCHAR ACSlowSlewH;			// non-zero if AC pins have slow slew
		printf("\tACSchmittInputH = 0x%X\n", Data.ACSchmittInputH);				//UCHAR ACSchmittInputH;		// non-zero if AC pins are Schmitt input
		printf("\tACDriveCurrentH = 0x%X\n", Data.ACDriveCurrentH);				//UCHAR ACDriveCurrentH;		// valid values are 4mA, 8mA, 12mA, 16mA
		printf("\tADSlowSlewH = 0x%X\n", Data.ADSlowSlewH);				//UCHAR ADSlowSlewH;			// non-zero if AD pins have slow slew
		printf("\tADSchmittInputH = 0x%X\n", Data.ADSchmittInputH);				//UCHAR ADSchmittInputH;		// non-zero if AD pins are Schmitt input
		printf("\tADDriveCurrentH = 0x%X\n", Data.ADDriveCurrentH);				//UCHAR ADDriveCurrentH;		// valid values are 4mA, 8mA, 12mA, 16mA
		printf("\tCbus0H = 0x%X\n", Data.Cbus0H);				//UCHAR Cbus0H;				// Cbus Mux control
		printf("\tCbus1H = 0x%X\n", Data.Cbus1H);				//UCHAR Cbus1H;				// Cbus Mux control
		printf("\tCbus2H = 0x%X\n", Data.Cbus2H);				//UCHAR Cbus2H;				// Cbus Mux control
		printf("\tCbus3H = 0x%X\n", Data.Cbus3H);				//UCHAR Cbus3H;				// Cbus Mux control
		printf("\tCbus4H = 0x%X\n", Data.Cbus4H);				//UCHAR Cbus4H;				// Cbus Mux control
		printf("\tCbus5H = 0x%X\n", Data.Cbus5H);				//UCHAR Cbus5H;				// Cbus Mux control
		printf("\tCbus6H = 0x%X\n", Data.Cbus6H);				//UCHAR Cbus6H;				// Cbus Mux control
		printf("\tCbus7H = 0x%X\n", Data.Cbus7H);				//UCHAR Cbus7H;				// Cbus Mux control
		printf("\tCbus8H = 0x%X\n", Data.Cbus8H);				//UCHAR Cbus8H;				// Cbus Mux control
		printf("\tCbus9H = 0x%X\n", Data.Cbus9H);				//UCHAR Cbus9H;				// Cbus Mux control
		printf("\tIsFifoH = 0x%X\n", Data.IsFifoH);				//UCHAR IsFifoH;				// non-zero if interface is 245 FIFO
		printf("\tIsFifoTarH = 0x%X\n", Data.IsFifoTarH);				//UCHAR IsFifoTarH;			// non-zero if interface is 245 FIFO CPU target
		printf("\tIsFastSerH = 0x%X\n", Data.IsFastSerH);				//UCHAR IsFastSerH;			// non-zero if interface is Fast serial
		printf("\tIsFT1248H = 0x%X\n", Data.IsFT1248H);				//UCHAR IsFT1248H;			// non-zero if interface is FT1248
		printf("\tFT1248CpolH = 0x%X\n", Data.FT1248CpolH);				//UCHAR FT1248CpolH;			// FT1248 clock polarity - clock idle high (1) or clock idle low (0)
		printf("\tFT1248LsbH = 0x%X\n", Data.FT1248LsbH);				//UCHAR FT1248LsbH;			// FT1248 data is LSB (1) or MSB (0)
		printf("\tFT1248FlowControlH = 0x%X\n", Data.FT1248FlowControlH);				//UCHAR FT1248FlowControlH;	// FT1248 flow control enable
		printf("\tIsVCPH = 0x%X\n", Data.IsVCPH);				//UCHAR IsVCPH;				// non-zero if interface is to use VCP drivers
		printf("\tPowerSaveEnableH = 0x%X\n", Data.PowerSaveEnableH);				//UCHAR PowerSaveEnableH;		// non-zero if using ACBUS7 to save power for self-powered designs
